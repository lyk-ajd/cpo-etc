...
// Dev: serve /landing/moving/<file> from ../../source/images/<file>.
// Build: copy all source/images/* into dist/landing/moving/ so the SFC's
// BASE_URL-aware cdn() helper resolves to <base>/landing/moving/<file>.
const movingImagesPlugin = {
  name: 'serve-moving-images',
  configureServer(server) {
    server.middlewares.use('/landing/moving', (req, res, next) => {
  ...
  closeBundle() {
    // Build hook — copy all source/images/* into dist/landing/moving/.
    const distLandingDir = path.resolve(here('./dist'), 'landing/moving')
...
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/cpo-etc/moving/' : '/',
  plugins: [vue(), movingImagesPlugin],
  resolve: {
    alias: [
