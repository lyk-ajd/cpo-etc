...
// Dev: serve /landing/cleaning/<file> from ../../source/images/<file>.
// Build: copy all source/images/* into dist/landing/cleaning/ so the SFC's
// BASE_URL-aware cdn() helper resolves to <base>/landing/cleaning/<file>.
const cleaningImagesPlugin = {
  name: 'serve-cleaning-images',
  configureServer(server) {
    server.middlewares.use('/landing/cleaning', (req, res, next) => {
  ...
  closeBundle() {
    // Build hook — copy all source/images/* into dist/landing/cleaning/.
    const distLandingDir = path.resolve(here('./dist'), 'landing/cleaning')
...
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/cpo-etc/cleaning/' : '/',
  plugins: [vue(), cleaningImagesPlugin],
  resolve: {
    alias: [
