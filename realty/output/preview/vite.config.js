...
// Dev: serve /landing/realty/<file> from ../../source/images/<file>.
// Build: copy all source/images/* into dist/landing/realty/ so the SFC's
// BASE_URL-aware cdn() helper resolves to <base>/landing/realty/<file>.
const realtyImagesPlugin = {
  name: 'serve-realty-images',
  configureServer(server) {
    server.middlewares.use('/landing/realty', (req, res, next) => {
  ...
  closeBundle() {
    const distLandingDir = path.resolve(here('./dist'), 'landing/realty')
...
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/cpo-etc/realty/' : '/',
  plugins: [vue(), realtyImagesPlugin],
  resolve: {
    alias: [
